        ...
        <div class="d-flex flex-column-fluid">
            ...
            <div class="container">
                ...
                <div class="card card-custom gutter-b example example-compact">
                    <form method="post" action="{{url(app()->getLocale().'/admin/cities/'.$item->id)}}"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control form-control-solid"
                                                       name="city_name_{{$locale->lang}}"
                                                       {{($locale->lang == 'ar') ? 'dir=rtl' :'' }} value="{{old('city_name_'.$locale->lang,@$item->translate($locale->lang)->city_name)}}"
                                                       required/>
                                            </div>
                                        </div>
                                    @endforeach
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            @foreach($countries as $country)
                                                    <option value="{{$country->id}}" {{$item->country_id==$country->id ? 'selected':''}}> {{@$country->country_name}} </option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
